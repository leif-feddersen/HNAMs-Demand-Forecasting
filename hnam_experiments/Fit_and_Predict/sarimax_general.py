...
def forecast_time_series(time_series_i):
    try:
        ...
        for first_test,end_test in zip(test_dates,test_end_dates):
            ...
            model = AutoARIMA(seasonal=True,season_length=SEASON_LENGTH, allowdrift=True,allowmean=True,approximation=True)
            model.fit(past_data['sales'].values,past_data[covs_i].values)

            while max_pred < end_test:
                preds = model.forward(y = past_data['sales'].values,
                                X = past_data[covs_i].values,
                                X_future = future_covs[covs_i].values[:PRED_LENGTH],
                                h = PRED_LENGTH,
                                level= conflevels)
                ...
                past_data = df_ts.query('date < @first_test & time_series == @time_series_i').drop('time_series',axis=1).set_index('date').asfreq('D').fillna(0).astype(float)
                future_covs = df_ts.query('date >= @first_test & date <= @end_test & time_series == @time_series_i')
                time_idx_i  = future_covs['time_idx'].min().astype(int)
                future_covs = future_covs.drop('time_series',axis=1).set_index('date').asfreq('D').fillna(0).astype(float)
